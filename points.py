...
def get_positions(filelist):
	...
	for filename in filelist:
		...
		event = filename.split('_')[2].split('.')[0]
...
def get_points(rposlistlist):
	...
	for race in rposlistlist:
		...
		for runner in sorted(race, key = lambda i: i['rpos']):
			if firstrunner == 0:
				if runner['rpos'] > 0:
					fasttime = runner['rpos']
					firstrunner = 1
			print(namelist)
			entry = next((item for item in namelist if item['name'] == runner['name']), None)
			print(entry)
			if runner['rpos'] > 0:
				score = round((fasttime / runner['rpos']) * 1000)
			else:
				score = 10
			print(score)
			if entry:
				entry[runner['racenum']] = score
			else:
				entry = {'name': runner['name'], runner['racenum']: score}
